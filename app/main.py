...
async def scrape_catalogue(
    ...
    email: str = Query(None, description="email if required"),
    to_number: str = Query(None, description="to_number if required"),
):
    
    scraped_data, new_products, updated_products = scraper.scrape_catalogue(pages=pages, proxy=proxy)
    notify(f"Scraping completed: {len(scraped_data)} products scraped-{len(new_products)} new products added and {len(updated_products)} products updated.", email, to_number)
